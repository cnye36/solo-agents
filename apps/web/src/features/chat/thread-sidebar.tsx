    ...
    <aside className="flex h-full min-h-0 flex-col overflow-hidden rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.045),rgba(255,255,255,0.02))] p-3 shadow-[0_20px_64px_rgba(0,0,0,0.22)] backdrop-blur">
      <div className="flex shrink-0 items-center gap-2">
        ...
        <button
          type="button"
          onClick={onNewThread}
          className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.04] text-slate-200 transition hover:border-white/20 hover:bg-white/[0.08] hover:text-white"
      ...
      <div className="mt-3 min-h-0 flex-1 overflow-y-auto pr-1">
        <div className="space-y-2">
          {filteredThreads.length ? (
            ...
            <div className="rounded-2xl border border-dashed border-white/10 bg-black/20 px-4 py-6 text-sm leading-6 text-slate-400">
              {threads.length
                ? "No matching threads."
                : "No threads yet."}
            </div>
